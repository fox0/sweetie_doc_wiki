...
Подсистема включает в себя несколько задатчиков объединенных общей (?)

Проводится "обучение" траектории, путем сохранения `JointState` для заданных контрольных точек (поза и время).
...
Хранение траекторий предлагается осуществлять за счет встроенных средств сериализации ROS. В одном файле --- одна траектория,
...
**Прагматика**: эти типы сообщений и действий используют действие по исполнению траектории используются `MoveIt!.`
...
Поле `path_tolerance` и `goal_time_tolerance` используются для проверки начальных условий и контроля ее исполнения. 
...
Предоставляет: действие `control_msg::FollowJointTrajectory`.
...
Задатчик `AnimationStoredJointTrajectory` (исполнение действия `FollowJointTrajectory` из файла)
----------------------------------------------------------------
...
Предоставляет: действие `TextAction` --- выбор траектории по имени.
...
Требует: `deployer_eval` (`string -> bool`) --- заставляет Deployer OROCOS исполнить заданный скрипт через сервис `rosdeployer`.
...
**Диалог "Trajectory setup..."** --- доступ к изменению базовых параметров траектории `FollowJointTrajectoryGoal` через текстовые поля, ввод через пробел
...
**"Execute virtual"** --- передает траекторию на исполнение задатчику виртуального робота. Результат по получению отображается рядом с кнопкой.
...
**"Execute real"** --- передает траекторию на исполнение задатчику реального робота аналогично пункту выше. Результат по получению отображается рядом с кнопкой.
