...
1. **Диалог "Trajectory setup..."** --- доступ к изменению базовых параметров траектории `FollowJointTrajectoryGoal`. 
    ...
    Алтернативный вариант --- через текстовые поля, ввод через пробел.

    В списке не может быть неизвестных звеньев.

2. **Список/таблица контрольных точек**: удаление точки, редактирование времени, редактирования списка значений углов (не особо надо), 
    ...
    Должны быть обеспечены средства контроля: список отсортирован по времени, маркер начала раньше конца и т.п. 
    В перспективе можно добавить отметки "реальная/моделируемая поза" в пределах `path_tolerance`.


3. **"Reverse direction"** --- меняет местами начальную точку и конечную (при наличии конпки "Execute real backward" не нужно).

1. **"Speed scale"** (слайдер), **"Apply scale"** --- первый устанавливает модификатор масштаба времени при исполнении (не меняет таблицу), второй применяет модификатор к таблице.
...
1. **"Save trajectory...", "Load trajectory..."** --- загрузка и сохранение траектории. Всегда сохраняется полное содержимое таблицы, от начала до конца, масшатб скорости исполнеения игнорируется.
