...
1. Задатчик `AnimationJointTrajectory` --- исполнение действия `control_msgs::FollowJointTrajectory`. 
    ...
    Набор необходимых ресурсов определяется по списку сочленений при помощи плагина [`RobotModelURDF`](plugin-robotmodel).
...
Для этого можно использовать средства сериализации OROCOS ([`boost::serialization`](http://www.boost.org/doc/libs/1_63_0/libs/serialization/doc/tutorial.html)) 
или ROS [rospy_message_converter](https://github.com/baalexander/rospy_message_converter).
...
2. Требует: сервис `RobotModel`.
2. Требует загрузки в компонент (опционально):  сервис `TransientJointStateInterface` сглаженная и сопряжения траекторий.
...
Считывает набор траекторий из параметров ROS. При получении сообщения с текстовым идентификатором нужной траектории 
...
1. `joints_ref` (`JointState`) --- задающее воздействие в угловой системе координат.
1. `support_ref` (`SupportState`) --- ожидаемое распределение веса, всегда ноль для управляемой конечности.

### Параметры

1. `string trajectoris_path` --- пространство имен ROS, используемое для хранения траекторий.
...
1. `support_state_override` (`SupportState`) --- список опорных ног.
...
1. `legs` (`string[]`) --- имена кинематических цепочек, способных опорными служить ногами.
...
1. **Диалог (вкладка) "Trajectory setup..."** --- доступ к изменению базовых параметров траектории `FollowJointTrajectoryGoal`. 
    ...
    В списке не может быть звеньев, отсутсвующих в `joint_states`.
...
3. *Управление списком опорных ног*. Флажок **"Publish contact list"** включает публикацию сообщения `SupportState`. Рядом список ног с флажками-признаками, что нога опорная. Список берется из параметра `legs`.
    Данный интерфейс используется для планирования сложных движений с перемещением платформы на виртуальном роботе. Он указывает компоненту одометрии, какие ноги считать опорными, что позволяет ему корректно вычисялять
    положение платформы. 
