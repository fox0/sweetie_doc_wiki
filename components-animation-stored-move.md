...
Эти возмжности в осоновном необходимы для разработки различных анимаций. Сначала робот "обучается" движению путем помещения в ключевые позы,
...
1. Предотавляет: действие `control_msg::FollowJointTrajectory`.
...
Задатчик `AnimationStoredJointTrajectory` (исполнение сохраненных траекторий)
----------------------------------------------------------------
...
1. Предотавляет: действие `TextAction` --- выбор траектории по имени.
