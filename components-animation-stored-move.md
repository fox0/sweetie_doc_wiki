...
Предлагается использовать подход с **совместным хранением** метаинформации и траектории с сериализацией в YAML посредством [yaml-cpp]().
...
Подобный подход примененн в [NimbRO-OP]().
...
Способ хранения траектории оптимизирован под ручное задание, указывается минимальный набор контрольных точек.
    ...
    trajectory: 
		 - point1:
               time: time # время от начала движения (c) (float)
...
это поле предлагается заполнять вручную.
...
Для интерполяции предлагается использовать `alglib` (`tinyspline` не подходит, т.к. не обеспечивает неравномерные временные интервалы).
...
который в дальнейшем требует ручного редактирования (описание, ресурсы, 
...
Код по непрерывному захвату сообщений можно разместить в `updateHook`, тогда периодичность регулируется `setPeriod`,
