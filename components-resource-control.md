...
1. `resource_assigment` (`ResourseList`) --- ресурсов по компонентам.

Внутренний интерфейс (связь с кодом задатчика):

1. Предоставляет: `bool startOperational(strings req [, ints pri1, ints pri2] )` --- изменить состав ресурсов, формирует запрос `ResourseRequest` и отправляет его арбитру. 
...
**startOperational** формирует запрос `ResourseRequest` и отправляет его арбитру. 
Эта функция вызывается кодом пользователя при активации, после проверки условий активации, сигнализирует о переходе в состояние ожидания активации (pending).

**stopOperational** транслирует в асинхронный вызов `requestResourcesChange`. Пользовательский код сигнализирует о деактивации.

**step** должна вызываться в начале `updateHook()` для обработки сообщенией `ResourseList`.
Фактически просто разбирает полученный список ресурсов и вызывает пользовательский `hookResourceChange`, 
...
**step** нужна для реализации на сообщениях, здесь можно обработать результаты `startOperational`. Альтернативный вариант --- сделать ее callback функцией для порта `resource_assigment`.
В любом случае для обработки сообщений компонент должен быть запущен, либо обработка сообщений должна быть явно разрешена через `dataOnPortHook`.
...
Основной код --- обработка запросов --- содержится в **`updateHook`**. Семантика исполнения подробно расписана в [переключении задатчиков](gait-switching).
Некорректный запрос приводит к ошибке.
