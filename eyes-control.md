...
Проблему может решить разными способами:
* выделение общей части анимации разных глаз в один модуль;
* детерминированное поведение (генератора случаных чисел с общим семенем для обоих глаз) и одновременный запуск;
* использование одного глаза в режиме "ведущий" с передачей направления взгляда и фазы моргания (самые важные парамтры анимации) остальным глазам.
В последнем случае парметризация изображения глаза не подходит, если в ней потеряно направление взгляда. 
Первый способ ведет к трехэтапной схеме построения изображения, поскольку общая для обоих глаз параметризация не позволит сделать анимацию, например, подмигивания.
Однако в случае использования разных отрисовщиков для разных глаз общего решенрия нет. 

В раздельной схеме можно разделить модуль кинематики, чтобы каждый из отрисовщикоа мог правильно пересчитывать направление взгляда в положение зрачка.
Информация в xml парметров также разделяется на два поддерева.
...
Наиболее спорное решение --- раздельный тракт. Это создает сложности при осуществления анимаций типа "подмигнуть глазом", 
не ясно, как сихронизировать отрисовщики анимаций разного типа.

### Открытые вопросы

* Нужна ли поддержка произвольного числа глаз в произвольных местах, меняется ли взаимное положение глаз? 
(Решается за счет массива отрисовщиков и модулей кинематики вместо двух, отражается на структуру конфигурационых фалов). 
* Нужен ли внешний интерфейс для независмого управления взглядом каждого глаза? (Усложняет интерфейс).
* Нужен список отображаемых эмоций и анимаций на первое вемя.
* Нужены ли самопроизвольные переключения анимаций (например, кончилась анимация подигивания, после нее продолжается предыдущая)?
