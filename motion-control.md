...
Агрегатор [`agregator_ref`](components-agregator-gait) формирует полную желаемую позу робота `q`, `dq` в угловой СК (`JointState`), также он агрегирует информацию о контактах от задатчиков (`SupportState`)
...
либо определяя самостоятельно предполагаемый список контактов, рассчитывает позу  `s` и скорость `w` платформы (тела) робота.
...
рассчитывают моменты приводов `tau` и силы реакции опоры `N` в точках контакта. Для расчетов динамики придется вычислять ускорения, численно дифференцируя скорость.
Динамическая модель робота загружается из URDF.

Компонент обращения приводов [`servo_inv_7param`](components-servo-inv) по желаемым позициям `q`, скоростям `dq` и расчетным усилиям приводов `tau` определяют команды для приводов, 
...
1. На первых этапах удобно обойтись без идентификатора `servo_ident` и загружать модель в xml файла методом сериализации OROCOS в параметр `servo_inv_7param`.

2. Скорость и ускорение платформы необходимы для расчета сил в системе со подвижной платформой.

3. В указанной схеме Якобиан надо вычислять несколько раз в разных модулях. Проблема может быть решена за счет сервиса прямой кинематики (?), либо можно публиковать якобиан на уровне агрегатора. 
...
1. Для вычисления управления необходимы ускорение `dw` платформы, ускорение свободных конечностей и список контактов. 
