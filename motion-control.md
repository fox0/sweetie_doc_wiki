...
Агрегатор `agregator_odom_ref` формирует полную желаему позу робота `q`, `dq` в угловой СК (`JointState`) и рассчетную скорость и позицию тела робота `s`, `w` в абсолютной СК.
...
Компонент обращения приводов `servo_inv_nimbro` по желаемым позициям `q`, скоростям `dq` и рассчетным усилиям приводов `tau` определяют команды для приводов, 
...
Компонент идентификации и рассчета ошибки пресказания `servo_ident` способен по внешней команде проводить идентификацию заданной группы приводов. 
