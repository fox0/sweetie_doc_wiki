...
В целом, интерфейс может быть индивидуальным для каждого задатчика.
...
т.к. скорость потенциально обновляется каждый цикл управления. 
Рекомендуемая схема состоит в использовании действия (ROS Action) для активации и деактивации задатчика (интерфейс реализованный в [`SimpleControllerBase`](components-gait)),
а управляющий сигнал поступает на входной порт непосредственно.
...
* Выходной порт с опорными коэффициентами (распределение веса `SupportState`). Для анимаций выходом почти всегда будет отсутствие касания.
...
1. Компонент-арбитр.
2. Клиентская часть (плагин, загружаемый в задатчик).

Принципиально возможно несколько реализаций механизма переключения: 
...
Поэтому, например, неясно как реализовывать переключение равноправных походок. Предлагается "гибрид" приоритетов и вытеснения:
запрашиваемые ресурсы снабжаются двумя приоритетами, один используется при запросе, а второй --- при нормальном функционировании.
Так активируемая походка может использовать высший приоритет при активации, а потом понижать его до среднего, чтобы другой 
задатчик мог перехватить управление.
...
На уровне клиентов и арбитра можно использовать более эффективные способы: `set`, `map`.
...
Задатчик может быть в 3-х состояние: активн (operational), активен и запрашивает ресурсы (operational pending), 
неактивен и запрашивает ресурсы (pending), неактивен (nonoperationl).
...
**Замечание**: Контроль отношений между `ResourceRequest`, `ResourceAssigment` и `ResourceRequesterState` при помощи информации о раунде необходим, чтобы избежать гонок при асинхронном взаимодействии. Ожидающий активации задачик должен реагировать только на сообщения арбитра, которые уже учитываюь его запрос. Аналогично, арбитр должен реагировать на `ResourceRequesterState`, только если оно старше последнего запроса полученного от задатчика.

Этого можно достичь следущим способом:
   * Запросы помечаются уникальным `request_id`, монотонно возрастающим с номером запроса (`uint32`старшая пара байт --- номер запроса, младшая --- хэш имени).  Такая структура позволяет получать `request_id` на стороне клиента без централизации. Альтернатива --- использовать номер запроса к арбитру, требует использования операций для посылки `ResourceRequest`, чтобы этот номер вернуть.
   * `ResourceAssigment` содержит `request_id` последних запросов всех компонент, состояние которых отлично от "не активен".
   * Свой `request_id` `ResourceAssigment` используется для пометки `ResourceRequesterState`.

Этот механизм позволяет легко определить, имеет ли арбитр актуальную информацию о задатке: последний полученный арбитром `request_id` должен быть равен идентификатору последнего запроса задатчика.

**Замечание**: Алгоритм не поддерживает ненадежную среду передачи сообщений. Для этого нужны timeout.

**Замечание**: Для передачи сообщений `ResourseRequest`, `ResourseList` и `ResourseRequesterState` можно использовать одно сообщение, просто объединив их поля и 
не заполняя ненужные поля при передаче. Однако делать это имеет смысл только для `ResourseRequesterState`  и `ResourseRequest`, чтобы сократить число портов и упростить развертывание.

#### Упрощенные реализации

* **Релизация без приоритетов с возвратом ресурсов прежднему владельцу**
    * Распределение ресурсов сводится к отъему у активных компонент нужных запрашивающему ресурсов.
    * По переходу задатчика в состояние "не активен" его ресурсы возвращаются предыдущему владельцу, т.е. фактически происходит обычный раунд распределения, 
         но роль приоритета играет порядковый номер запроса. 
...
Операция используется для `ResourseRequest`: арбитр возвращает номер раунда задатчику. Потенциально, это может быть быстрая ClientThread с буферизацией запросов.
...
Клиент переносит номер раунда в `ResourceRequesterState` из `ResourceAssigment`, что позволяет арбитру узнать, было ли обработано конкретное распределение ресурсов.

**Достоинства**:

1. Очень простой контроль раундов сообщений.
...
Сильно усложняется контроль раундов. Уникальные идентификаторы раундов генерируются на уровне клиента. 
Идентификатор состоит из двух полей: имя клиента и порядковый номер запроса клиента.

`ResourceAssigment` должен содержать список идентификаторов, формируемый по следующим правилам.
* Идентификатор запроса попадает в список, если обработан соответствующий ему `ResourseRequest`.
* Идентификатор запроса удаляется из список, если обработан соответствующий ему `ResourceRequesterState`.
...
* при выходе из состояния pending: идентификатор последнего запроса клиента,
* при выходе из состояния operational: нулевой идентификатор.
ИЛИ
* номер раунда арбитра из `ResourceAssigment`
Это информация позволяет арбитру понять, обработано ли распределение всеми компонентами в стоянии `pending`.
...
Не рассматривается, т.к. не обеспечивает целевую асинхронность.
