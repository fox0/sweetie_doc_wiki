...
этого типа. Если один из них формирует `ResourceRequest`, он через `resourse_control` передается всем другим задатчикам.

* **Активация задатчика** происходит внешним воздействием (операция, сообщение). Компонент проверяет условия запуска. 
Если они не удовлетворены, то в результате операции, или отдельным сообщением, или через механизм `actionlib`, осуществляется извещение о неудаче. 
Иначе формируется сообщение `ResourceRequest` со списком необходимых ему ресурсов, активация сичтается успешной при получении эха собственного сообщения.
...
* **`resourse_control`** просто формирует эхо полученных сообщений.
...
0. Асинхронность: возможна ситуация, когда два задатчика одновременно думают, что активны. Длительность этого промежутка определяется временем доставки сообщений и их обработки. Проблема несущественна из-за скорости этих процессов.
...
1. `register(string name)` --- регистрация задатчика, имя и операция `release` добавляется в список. Может быть реализована через параметры.
...
Недостаток синхронности может быть снят, если извещать только активные задатчики.
