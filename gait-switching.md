...
Что позвояет сразу же получить операции запуска, останова, извещения о состоянии (не получилось, в работе, завершено). 
...
Для проверки условий запуска без передачи управления задатчико можно ввести отдельную операцию.


Взаимодействие с подсистемой управления движения
-----------------------------------------------

* Входные порты с разого типа: текущей поза робота (желаемая или измеренная), датчики касния, прочее.
...
Вероятно, вытеснение на базе операций --- самый адекватный результат.
...
Предлагается вариант 1. Он не отменяет возможнось создания специального класса для ускорения операций над наборами ресурсов.
...
Не обеспечивает возможность перехвата вспомогательных ресурсов, т.е. все ресурсы предполагаются необходимыми.

Взаимодействие осуществляется сообщениями `ResourceRequest`. Каждый задатчик снабжен входным и выходным портом 
этого типа. Если один из них формирует `ResourceRequest`, он через `motion_switch` передается всем другим задатчикам.

* **Активация задатчика** происходит внешним воздействием (операция, сообщение). Компонент проверяет условия запуска. 
Если они не удовлетворены, то в результате операции, или отдельным сообщением, или через механизм `actionlib` осуществляется извещение 
о неудаче. Иначе формируется сообщение `ResourceRequest` со списком необходимых ему ресурсов, 
активация сичтается успешной при получении эха собственного сообщения.

* **Деактивация задатчика** происходит либо по внешнему (высший уровень) или внутреннему событию (конец движения), 
либо по получению сообщения `ResourceRequest`, содержащего запрос на необходимые данному компонету ресурсы, но не посланное им.

* **`motion_switch`** просто формирует эхо полученных сообщений.

**Недостатки**:

1. Не реализует временный перехват ресурсов.
2. Потеря сообщений при передаче: нужная походка может не выключиться, если происходит одноверменное переключение (решается буфферизацией сообщений).
3. Недетерминированное поведение при попытке одновременного включения конфликтующих походок (решается буфферизайией и активацией по получению эха `ResourceRequest`).
...
Переполнение буффера сообщений (потеря сообщения) приводит к недеретминированному поведению.
Буфер входящих сообщений должен быть очищен перед запросом ресурсов.
...
Задатчики предоставляют операцию `void release(const ResourseRequest&)`, семантика которой совпадает с получением сообщения `ResourseRequest`.

`motion_switch` предоставляет операции:

1. `void register(string name)` (OwnThread) --- регистрация задатчика, имя и операция `release` добавляется в список.

2. `void request(const ResourseRequest& req)` (OwnThread) --- эквивалент эха. Последовательно вызываются операции `release` для всех зарегистрированных компонент,
кроме вызвавшего операцию (он передает свое имя в `req`). 
...
Благодаря синхронности работы операций исчезает ряд потенциальных проблем.


Реализация с возможотью перехвата ресурсов: приоритеты для `motion_switch`
--------------------------------------------------------------------------
