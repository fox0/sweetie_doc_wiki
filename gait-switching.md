...
Это наиболее простая реализация, удовлетворяющая большинству приведенных выше требований. 
...
* **Деактивация задатчика** происходит по внешнему событию (команда с высшего уровня), внутреннему событию (конец движения) или в результате вытеснения другим задатчиком.
...
Недетерминированность решается буферизацией. В правильной реализации активируется задатчик, пославший сообщение запроса последним.
Переполнение буфера сообщений (потеря сообщения) приводит к недетерминированному поведению. Если компонент послал запрос на активацию,
то он должен игнорировать входящие `ResourceRequest`, пока не получает эхо своего запроса, либо истечет период ожидания.

Базовая реализация: механизм вытеснения (операции)
--------------------------------------------------

Тот же механизм может быть реализован на база операций. 
Синхронная природа операций и встроенная буфферизация немут ряд положительных эффектов. 
Однако усложняется механизм извещения активных компонент о выделении ресурсов, т.к. арбитр должен индивидуально устанавливать 
соединение с операциями всех задатчиков.
...
Логика работы аналогична реализации с портами. Посылки сообщения `ResourseRequest` соответствует вызов `request`, после которого ресурсы можно считать захваченными.
...
2. Не требует надеется на буфферизацию сообщений.
...
Реализация с возможностью перехвата ресурсов: приоритеты для `resource_control`
--------------------------------------------------------------------------
...
*Вероятный вердикт*: слишком сложно.


Реализация с возможностью перехвата ресурсов: приоритеты для `agregator_`
-----------------------------------------------------------------------

В выходные сообщения задатчиков `JointLimbState`, `CartesianState` и др. к каждому ресурсу добавляется поле приоритета.
