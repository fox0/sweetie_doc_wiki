...
### Имена пкетов, каталогов и файлов

**Имена пакетов** всегда содержат префикс `sweetie_bot`, далее следует характеристика подсистемы, объединяюей несколько компонент. 
Из-за специфики создания типов OROCOS придется хранить сообщения отдельно от использующих их пакетов. Для создания иерархии используется система подкаталогов.
Использование префиксов в именах каталогах --- лишнее, т.к. ROS их не обрабатывает.

    sweetie_bot 
     +- sweetie_bot_logger --- пакет верхнего уровня
     +- motion --- подсистема управления движением РВ
     |   +- controller --- походки и анимации, их библиотеки.
     |   +- sweetie_bot_resource_control --- арбитр ресурсов
     |   +- sweetie_bot_agregator
     +- motion_msgs --- сообщения OROCOS
     |   +- sweetie_bot_resource_control_msgs --- сообщения подсистемы арбитра ресурсов
     |   +- rtt_sweetie_bot_resource_control_msgs --- typekit сообщений подсистемы арбитра ресурсов
     +- eyes 
     |   +- sweetie_bot_eyes 
     +- hmi --- человеко-машинный интерфейс (джойстики, визуализация).
     +- deployment
     |   +- sweetie_bot_deployment
     |   +- sweetie_bot_deployment_tests --- тесты
     +- hardware
         +- sweetie_bot_herkulex_control
         +- sweetie_bot_herkulex_msgs
         +- rtt_sweetie_bot_herkulex_msgs
  
Правила закрепляют сложившуюся структуру пакетов. Имя пакета фактически представляет пространство имен.
Единственное исключение --- `motion`.  Переход на именование с префиксом `sweetie_bot_motion` потребует слишком моного усилий и сильно удлинит имена пакетов. 
...
**Имя загруженного компонента OROCOS**. Т.к. в OROCOS нет пространст имен, то включает перфикс подсистемы.

*Идея*: имя кмпонента OROCOS --- это строка, поэтому можно использовать имена вида `herkulex/array` имитирующие пространства 
имен ROS. При загрузке параметров ROS `/` воспринимается, как разделитель, что позволяет иерархически организовать парметры
по подсистемам. Также удается установить полное соответсвие с категорями журналирования. Единственная проблема --- обращения к компонентам из скриптов ops --- решается использованием `aliasPeer`.

    resource_control_arbiter --- арбитр ресурсов,
    agregator_ref --- агрегатор желаемой позы (выход задатчиков),
    agregator_real --- агрегатор реальной позы,
    herkulex_array --- контроль приводов,
    gait_trot_gait --- еще одна походка (без префикса),
...
**Имя загруженного компонента ROS** помещаетя в пространств имен дублирующую схему для пространст имен выше.
    ...
    sweetie_bot
     ...
     |   +- herkulex_array (компонент)
     |   +- resource_control
     |   +- servo_inv
     +- motion_virtual (нода rttlua, OROCOS)
     |   +- resource_control
     +- robot_state_publisher
     +- robot_state_publisher_virtual
...
Примеры:
   ...
   sweetie_bot.motion.resource_control.resource_control_arbiter
