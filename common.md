...
Трекер gitlab предоставляет возможность группировки задач (`issue`) при помощи меток (`label`). Рользователь может удобно настроить интерфейс для отображения задач по группам, 
определяемым метками. Для придания удобства работа с трекером предлагается придерживаться следующих соглашений.

**Milestones** --- ближайшие объемлющие цели проекта: первый релиз, демонстрация для РБК-17 и т.д. Ткакая цель включает несколько задач.

Задачи (`issues`) при помощи меток на 3 типа:
* **task** --- объемная задача по реализации, например, пакета c несколькими взаимосвязанными компонентами. Задача может находиться в состояниях **design**, **realization**, **testing**, **done**. 
    В комментариях можно протоколировать основные рещения, проводить дискуссию и т.п.
* **feature** --- небольшая задача по добавлению какой-нибудь функциональности.
* **bug** --- жук, их будет много.

Также задачи делятся по подсистемам робота (если это возможно): **documentation**, **hardware**, **eyes**, **motioncontrol**, **animation**, **gait**, **highlevel** (список продолжить).
...
Для компонент OROCOS описания сообщений (`msg`, `actionlib`) выделяются в отдельные пакеты, их имя совпадает с именем исходного пакета с добавлением постфикса `_msg`.
Для компонент ROS описания сообщений хранятся в том же пакете.
...
различных версий кинематики). Сообщения OROCOS должны быть выделены из-за особенностей сборки typekits.
...
* Не следует включать в имя файла префиксы, подобные используемым в именах пакетов. Файлы уже и так изолированы файловой системой и пакетами. Например, следует использовать `herkulex_sched-component.cpp` вместо `sweetie_bot_herkulex_sched-component.cpp`. 
...
Прелагается следующая **иерархия пространств имен** (дополняетс по мере необходимости):

    sweetie_bot --- все, что относится непосредственно к роботу.
     +- motion (бывшее core) --- подсистема управления движением РВ
     |   +- controller --- походки и анимации, их бибилиотеки.
     |   +- общие компоненты подсистемы управления движением.
     +- eyes 
     +- hw --- аппартно-зависимые части, напрямую связанные с роботом.
     |   +- proto2
     |   +- proto3
     |   +- beagle_board 
     +- hmi --- человекомашинный интерфейс (джойстики, визуализация).
     +- deployment
     ...
    herkulex --- подсистема, которую можно использовать полностью независмо от робота.

Эта иерархия полностью или частично дублируется структурой каталогов проекта.  
...
Следуем рекомендациям ROS. Вспосогательные скрипты и файлы параметров размещаются в `scripts`.
...
Тестовый скрипт также выпоняет роль примера использования для последующей интеграции пакета.
...
1. RTT::Logger --- штатное средство, не поддерживает категории, т.е. лог общий для всех. Рекомендуется только использование для вывода сообщений пользователю  и ограниченно для отладочных сообщений, выводимых *незначительное* число раз при отладке компонена.

2. G&P Logger --- часть проекта, поддерживает категори, реальное время, /rosout. Использовать всегда при отладочном выводе, идущем "потоком". Менее удобен для сообщений пользователю. Рекомендуемая опорная инфраструктура: OCL или rosout.
...
* Любая нештатная ситуация, характеризующаяся неспособности выполнить запрощенное действие, должна приводить к сообщению в журнале. 
    Уровни сообщений: `CRIT` (работа подсистемы не мжет быть продолжна), `ERROR` (подсистема осталась работоспособна).
...
Следует стараться, чтобы внешний интерфейс сервисов и компонет содержать только элементы,
предназначенные для испольщования из вне (пользователем и другими компонентами).
В целях инкапсуляции туда не следует добавлять элементы, ктороые могут нарушать 
внутреннее состояние компонета или имеют смысл только в его внутреннем контексте.

Соответсвенно,  *предпочтительный способ взаимодействия с элементами сервиса, не предназначенными для 
