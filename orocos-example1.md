...
* Используя `addEventPort`, можно назначить отдельный callback для обработки событий получения данных. Однако тогдa `upadateHook` не вызывается.
    По умолчанию callback вызывается только для запущенных компонент, но это поведение может быть изменено путем реализации `dataOnPortHook()`.
