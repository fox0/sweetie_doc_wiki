...
С каждой из стрелок ассоциирован конкретны [тип сообщений](Типы сообщений).
Блоки соответствуют определенным действиям над переменными состояния. Они инкапсулированным в отдельных модулях --- компонентах. 
...
2. Сообщения `giagnostic_msg`.
    ...
    Дополнение к основному механизму, использование обязательно когда тебуется известить верхний уровень о ошибке. 
...
Любые исключения не являются РВ механизм, логгирования тоже не является РВ операцией, допускается использование только лога `realtime`.

Уровни ошибочных ситуаций:

1. **Предупреждение** (`warning`) --- подозрительная ситуация, не влияющия на работу компонента существенно (неправильное имя, лишняя конечность и т.п.)
