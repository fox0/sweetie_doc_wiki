...
2. **Скрипты высокого уровня.** Они загружают нужный набор параметров и модулей, включая запуск подсистемы OROCOS на на роботе или компьютере оператора.
     ...
     * `run_real` (boolean) означает, что будут запущены компоненты исполнительной системы робота (управление приводами, экраны-глаза).
     * `robot` (boolean) указывает, что будут запущены компоненты, которые должны исполнятся на бортовом компьютере робота.
     * `host` (boolean) указывает, что будут запущены компоненты, кторые должны исполнятся на машине оператора.
